...
public:
	...
	void SpawnObjectExtras(LocationPart& locPart, BaseObject* obj, const Vec3& pos, float rot, void* user_ptr, float scale = 1.f, int flags = 0);
	void ProcessBuildingObjects(LocationPart& locPart, City* city, InsideBuilding* inside, Mesh* mesh, Mesh* inside_mesh, float rot, GameDirection dir,
	...
	ObjectEntity SpawnObjectNearLocation(LocationPart& locPart, BaseObject* obj, const Vec2& pos, float rot, float range = 2.f, float margin = 0.3f,
		float scale = 1.f);
	ObjectEntity SpawnObjectNearLocation(LocationPart& locPart, BaseObject* obj, const Vec2& pos, const Vec2& rot_target, float range = 2.f, float margin = 0.3f,
		float scale = 1.f);
	...
	Unit* SpawnUnitNearLocation(LocationPart& locPart, const Vec3& pos, UnitData& unit, const Vec3* look_at = nullptr, int level = -1, float extra_radius = 2.f);
	...
	void SpawnUnitsGroup(LocationPart& locPart, const Vec3& pos, const Vec3* look_at, uint count, UnitGroup* group, int level, delegate<void(Unit*)> callback);
	...
	void CreateBlood(LocationPart& locPart, const Unit& unit, bool fully_created = false);
	void SpawnBlood();
	...
	void WarpNearLocation(LocationPart& locPart, Unit& uint, const Vec3& pos, float extra_radius, bool allow_exact, int tries = 20);
	...
	bool CanSee(LocationPart& locPart, const Vec3& v1, const Vec3& v2, bool is_door = false, void* ignored = nullptr);
