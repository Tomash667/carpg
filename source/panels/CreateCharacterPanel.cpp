...
void CreateCharacterPanel::UpdateUnit(float dt)
{
	...
	if(anim != anim2)
	{
		...
		case DA_ATTACK:
			...
			mesh_inst->Play(NAMES::ani_attacks[unit->attack_id], PLAY_PRIO1 | PLAY_ONCE, 1);
			mesh_inst->groups[1].speed = unit->GetAttackSpeed();
		...
		case DA_SHOOT:
			...
			mesh_inst->groups[1].speed = unit->GetBowAttackSpeed();
			unit->animation_state = 0;
			...
			unit->bow_instance->groups[0].speed = mesh_inst->groups[1].speed;
			unit->action = A_SHOOT;
			t = 100.f;
			break;
		case DA_SHOW_WEAPON:
