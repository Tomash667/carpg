...
void Game::UpdateGameDialogClient()
{
	...
	{
		int prevChoice = dialog_context.choice_selected;
		if(!dialog_context.showChoicesComplete && game_gui->level_gui->UpdateChoice(dialog_context, dialog_choices.size()))
		{
			NetChange& c = Add1(Net::changes);
			c.type = NetChange::CHOICE;
			c.id = dialog_context.choice_selected;

			dialog_context.choice_selected = prevChoice;
			dialog_context.showChoicesComplete = true;
		}
	}
