...
struct NetChange
{
	enum TYPE
	{
		...
		PARTICLE_EFFECT, // create particle effect [int(ability->hash), Vec3(pos), Vec2(extraFloats)-bounds]
		...
		PLAYER_ABILITY, // player unit is using ability SERVER[int(id)-unit, int(ability->hash), float(extraFloat)-speed] / CLIENT[int(id)-unit, Vec3-pos/data, int(ability->hash)]
