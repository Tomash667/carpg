...
struct NetChange
{
	enum TYPE
	{
		...
		CHANGE_EQUIPMENT, // unit changed equipped item SERVER[int(id)-unit, byte(id)-item slot, Item-item(automaticaly set)] / CLIENT[int(id)-iIndex (if slot remove, else equip)]
		...
		ATTACK, // unit attack SERVER[int(id)-unit, byte(id)-type/flags, float(f[1])-speed] / CLIENT[byte(id)-type/flags, float(f[1])-speed, auto: Vec3 - for shoot targetPos]
		...
		DROP_ITEM, // unit drops item SERVER[int(id)-unit] / CLIENT[int(id)-iIndex, int(count)-count]
		...
		SHOOT_ARROW, // create shooted arrow [int-bullet id, int-owner id, Vec3-pos, float-rotX, float-rotY, float-speed, float-yspeed, uint-ability hash]
		LOOT_UNIT, // player wants to loot unit [int(id)-unit]
		GET_ITEM, // player gets item from unit or container [int(id)-iIndex, int(count)-count]
		GET_ALL_ITEMS, // player picks up all items from corpse/chest []
		PUT_ITEM, // player puts item into unit or container [int(id)-iIndex, int(count)-count]
		...
		TELL_NAME, // hero tells his name [int(id)-unit, bool(id)-setName, string1-name if set]
		...
		REGISTER_ITEM, // register new item [auto: string1(baseItem.id)-item id, string1(item2.id)-id, string1(item2.name)-name, string1(item2.desc)-description, int(item2.questId)-item quest]
		...
		RENAME_ITEM, // item rename [auto: int(baseItem.questId), string1(baseItem.id), string1(baseItem.name)]
		...
		CHEAT_SPAWN_UNIT, // player used cheat 'spawnUnit' [string1(baseUnit)-unit, byte(count)-count, char(id)-level, char(i)-inArena]
		CHEAT_ADD_ITEM, // player used cheat 'addItem' or 'addTeamItem' [string1(baseItem)-item id, byte(count)-count, bool(id)-is team]
		CHEAT_SET_STAT, // player used cheat setStat [byte(id)-stat id, bool(count)-is skill, char(i)-value]
		CHEAT_MOD_STAT, // player used cheat modStat [byte(id)-stat id, bool(count)-is skill, char(i)-value]
		CHEAT_GOTO_MAP, // player used cheat 'gotoMap' []
		...
		IS_BETTER_ITEM, // client checks if item is better for npc SERVER[bool(id)-is better] / CLINT[int(id)-iIndex]
		PVP, // response to pvp request [bool(id)-is accepted]
		CHEAT_REVEAL_MINIMAP, // player used cheat 'revealMinimap'
		...
		CAST_SPELL, // unit cast spell SERVER[int(id)-unit, uint(ability->hash)] / CLIENT[Vec3(pos)-target pos]
		CREATE_SPELL_BALL, // create ball - spell effect [int-abilityHash, int-id, int-ownerId, Vec3-pos, float-rotY, float-yspeed]
		SPELL_SOUND, // play spell sound [int(extraId)-type (0-cast, 1-hit), uint(ability->hash), Vec3(pos)]
		...
		PARTICLE_EFFECT, // create particle effect [uint(ability->hash), Vec3(pos), Vec2(extraFloats)-bounds]
		...
		CHANGE_MP_VARS, // multiplayer vars changed [auto: bool-mpUseInterp, float-mpInterp]
		...
		PLAYER_ABILITY, // player unit is using ability SERVER[int(id)-unit, uint(ability->hash), float(extraFloat)-speed] / CLIENT[int(id)-unit, Vec3-pos/data, int(ability->hash)]
		END_FALLBACK, // client fallback ended []
		RUN_SCRIPT, // run script [string(str)-code, int(id)-target unit]
		SET_NEXT_ACTION, // player set next action [auto: byte-nextAction, ...]
		...
		CRAFT, // craft item [uint(recipe->hash), uint(count)]
