		...
		case NetChange::ATTACK:
			{
				...
				else if(game->game_state == GS_LEVEL)
				{
					...
					case AID_Attack:
						...
						{
							const float ratio = meshInst->groups[1].time / unit.GetAttackFrame(0);
							unit.animation_state = AS_ATTACK_CAN_HIT;
							meshInst->groups[1].speed = (ratio + unit.GetAttackSpeed()) * unit.GetStaminaAttackSpeedMod();
						...
						else
						{
							...
							meshInst->groups[1].speed = attack_speed;
						}
					...
					case AID_StartShoot:
						...
						else
						{
							...
							MeshInstance* bowMeshInst = unit.GetBowMeshInstance();
							bowMeshInst->Play(&bowMeshInst->mesh->anims[0], PLAY_ONCE | PLAY_PRIO1 | PLAY_NO_BLEND, 0);
							bowMeshInst->groups[0].speed = meshInst->groups[1].speed;
					...
					case AID_Block:
						...
						meshInst->groups[1].speed = 1.f;
						meshInst->groups[1].blend_max = attack_speed;
						break;
					case AID_Bash:
						unit.action = A_BASH;
					...
					case AID_Cancel:
						...
						{
							unit.GetBowMeshInstance()->Deactivate();
							is_bow = true;
						}
						unit.action = A_NONE;
		...
		case NetChange::IDLE:
			{
				...
				{
					unit.node->mesh_inst->Play(unit.data->idles->anims[index].c_str(), PLAY_ONCE, 0);
				...
				if(state == USE_USABLE_START)
				{
					...
					else
					{
						...
						if(!IsSet(base.use_flags, BaseUsable::CONTAINER))
						{
							...
							unit.animation = ANI_PLAY;
							unit.node->mesh_inst->Play(base.anim.c_str(), PLAY_PRIO1, 0);
		...
		case NetChange::PLAYER_ABILITY:
			{
				...
				if(!ability)
				{
					Error("Update server: PLAYER_ABILITY, invalid ability %u.", ability_hash);
					break;
				}
				if(game->game_state == GS_LEVEL)
				{
				...
				case Shortcut::TYPE_ABILITY:
					{
						...
						if(!f)
							break;
						value = (int)Ability::Get(ability_hash);
						if(value == 0)
						{
							Error("Update server: SET_SHORTCUT invalid ability %u from %s.", ability_hash, info.name.c_str());
							ok = false;
						}
					}
					break;
