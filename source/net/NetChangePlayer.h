...
struct NetChangePlayer
{
	enum TYPE
	{
		...
		ADD_PERK, // add perk to player [uint(id)-perk hash, char(count)-value]
		REMOVE_PERK, // remvoe perk from player [uint(id)-perk hash, char(count)-value]
		...
		ADD_RECIPE, // add recipe to player [uint(recipe->hash)]
