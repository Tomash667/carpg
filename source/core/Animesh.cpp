...
void Animesh::Load(HANDLE file, IDirect3DDevice9* device)
{
	...
	if(IS_SET(head.flags,ANIMESH_ANIMATED) && !IS_SET(head.flags,ANIMESH_STATIC))
	{
		...
		// wczytaj animacje
		{
			...
			// dodaj zerow¹ koœæ do liczby koœci
			++head.n_bones;
			//++n_real_bones;
		}
	}
	...
	if(head.n_points > 0)
	{
		...
		for(word i=0; i<head.n_points; ++i)
		{
			...
			f >> p.bone;
			if(head.version == 12)
				++p.bone; // tu trzeba dodaæ bo tam liczy od zera
			f >> p.type;
			if(head.version >= 14)
				f >> p.size;
			else
			...
			{
				// fallback, zazwyczaj nie dzia³a ale wczeœniej tak by³o (dobrze dzia³a dla PI/2 i PI*3/2, dla 0 i PI jest odwrócone, dla reszty Ÿle)
