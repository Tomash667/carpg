...
void PlayerController::UseAbility(Ability* ability, bool from_server, const Vec3* pos, Unit* target)
{
	...
	if(ability->type == Ability::Charge)
	{
		...
		meshInst->Play(NAMES::ani_run, PLAY_PRIO1 | PLAY_NO_BLEND, 0);
		if(dash)
		{
			unit->timer = 0.33f;
			unit->speed = ability->range / 0.33f;
			meshInst->groups[0].speed = 3.f;
		}
		else
		{
			unit->timer = 0.5f;
			unit->speed = ability->range / 0.5f;
			meshInst->groups[0].speed = 2.5f;
			meshInst->groups[1].blend_max = 0.1f;
			meshInst->Play("charge", PLAY_PRIO1, 1);
		}
...
void PlayerController::UpdateMove(float dt, bool allow_rot)
{
	...
	MeshInstance* meshInst = u.node->mesh_inst;
		...
		&& (GKey.KeyPressedReleaseAllowed(GK_USE) || (u.IsNotFighting() && GKey.KeyPressedReleaseAllowed(GK_ATTACK_USE))))
	{
		...
		else if(data.before_player == BP_ITEM)
		{
			...
			if(u.action == A_NONE)
			{
				...
				u.action = A_PICKUP;
				u.animation = ANI_PLAY;
				meshInst->Play(up_anim ? "podnosi_gora" : "podnosi", PLAY_ONCE | PLAY_PRIO2, 0);
		...
		if(u.weapon_taken == W_ONE_HANDED)
		{
			if(u.action == A_ATTACK)
			{
				if(u.animation_state == AS_ATTACK_PREPARE)
				{
					...
					{
						// release attack
						const float ratio = meshInst->groups[1].time / u.GetAttackFrame(0);
				...
				else if(u.animation_state == AS_ATTACK_FINISHED)
				{
					...
					if(k != Key::None)
					{
						...
						meshInst->Play(NAMES::ani_attacks[u.act.attack.index], PLAY_PRIO1 | PLAY_ONCE, 1);
						meshInst->groups[1].speed = speed;
						action_key = k;
						u.timer = 0.f;
			...
			else if(u.action == A_BLOCK)
			{
				...
				else if(!meshInst->groups[1].IsBlending() && u.HaveShield())
				{
					if(GKey.KeyPressedUpAllowed(GK_ATTACK_USE) || GKey.KeyPressedUpAllowed(GK_SECONDARY_ATTACK))
					{
						...
						meshInst->Play(NAMES::ani_bash, PLAY_ONCE | PLAY_PRIO1, 1);
						meshInst->groups[1].speed = speed;
				...
				|| (u.action == A_ATTACK && !u.act.attack.run && !(u.animation_state == AS_ATTACK_CAN_HIT && u.act.attack.power > 1.5f))))
			{
				...
				if(k != Key::None)
				{
					...
					u.action = A_BLOCK;
					meshInst->Play(NAMES::ani_block, PLAY_PRIO1 | PLAY_STOP_AT_END, 1);
					meshInst->groups[1].blend_max = blend_max;
					action_key = k;
		...
		else
		{
			...
			if(u.action == A_SHOOT)
			{
				if(u.animation_state == AS_SHOOT_PREPARE)
				{
					...
					{
						meshInst->Deactivate(1);
						u.GetBowMeshInstance()->Deactivate();
						u.action = A_NONE;
						data.wasted_key = action_key;
						if(Net::IsOnline())
						{
			...
			else if(u.frozen == FROZEN::NO)
			{
				...
				if(k != Key::None)
				{
					...
					action_key = k;
					MeshInstance* bowMeshInst = u.GetBowMeshInstance();
					bowMeshInst->Play(&bowMeshInst->mesh->anims[0], PLAY_ONCE | PLAY_PRIO1 | PLAY_NO_BLEND, 0);
					bowMeshInst->groups[0].speed = speed;
	...
	if(idle && u.action == A_NONE)
	{
		...
		if(idle_timer >= 4.f)
		{
			...
			else
			{
				...
				meshInst->Play(u.data->idles->anims[id].c_str(), PLAY_ONCE, 0);
