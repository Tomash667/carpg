...
void Minimap::Draw(ControlDrawData* /*cdd*/)
{
	...
	if(type == L_DUNGEON || type == L_CRYPT || type == L_CAVE)
	{
		...
		if(inside->HaveDownStairs() && IS_SET(lvl.mapa[lvl.schody_dol.x+lvl.schody_dol.y*lvl.w].flagi, Pole::F_ODKRYTE))
			GUI.DrawSprite(game.tSchodyDol, INT2(TileToPoint(lvl.schody_dol))-INT2(16,16), COLOR_RGBA(255,255,255,180));
		if(inside->HaveUpStairs() && IS_SET(lvl.mapa[lvl.schody_gora.x+lvl.schody_gora.y*lvl.w].flagi, Pole::F_ODKRYTE))
