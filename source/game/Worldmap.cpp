		...
		case L_CRYPT:
			{
				...
				LOG(Format("Generating dungeon, target %d.", inside->cel));
				GenerateDungeon(l);
			}
			break;
		case L_CAVE:
			...
			if(location->active_quest && location->active_quest != (Quest_Dungeon*)ACTIVE_QUEST_HOLDER && !location->active_quest->done)
			{
				...
				if(location->active_quest->unit_to_spawn)
				{
					...
					if(u)
					{
						...
						DEBUG_LOG(Format("Generated quest unit %s.", u->data->id, u->GetName()));
					}
				}
			}
					...
					if(event)
					{
						if(!event->done && dungeon_level == event->at_level)
						{
							...
							if(event->unit_to_spawn)
							{
								...
								DEBUG_LOG(Format("Generated unit %s (%g,%g).", event->unit_to_spawn->id, spawned->pos.x, spawned->pos.z));
							}
							if(event->spawn_item == Quest_Dungeon::Item_GiveStrongest)
							{
								...
								if(best)
								{
									...
									DEBUG_LOG(Format("Item %s given to %s (%g,%g).", event->item_to_give[0]->id, best->data->id, best->pos.x, best->pos.z));
								}
							}
							else if(event->spawn_item == Quest_Dungeon::Item_GiveSpawned)
							{
								...
								if(spawned)
								{
									...
									DEBUG_LOG(Format("Item %s given to %s (%g,%g).", event->item_to_give[0]->id, spawned->data->id, spawned->pos.x, spawned->pos.z));
								}
							}
							else if(event->spawn_item == Quest_Dungeon::Item_OnGround)
							{
								...
								DEBUG_LOG(Format("Generated item %s on ground (%g,%g).", event->item_to_give[0]->id, item->pos.x, item->pos.z));
							}
						}
				...
				if(event)
				{
					if(!event->done && dungeon_level == event->at_level)
					{
						...
						else if(event->spawn_item == Quest_Dungeon::Item_OnGround)
						{
							...
							DEBUG_LOG(Format("Item %s spawned at ground (%g,%g).", event->item_to_give[0]->id, item->pos.x, item->pos.z));
						}
					}
					location_event_handler = event->location_event_handler;
				}
...
void Game::GenerateStockItems()
{
	...
	int price_limit, price_limit2, count_mod, div_mod;
...
void Game::GenerateDungeon(Location& _loc)
{
	...
	if(!IS_SET(base.options, BLO_LABIRYNTH))
	{
		...
		lvl.w = lvl.h = opcje.w;
		lvl.mapa = opcje.mapa;
		lvl.schody_gora = opcje.schody_gora_pozycja;
		lvl.schody_gora_dir = opcje.schody_gora_kierunek;
		lvl.schody_dol = opcje.schody_dol_pozycja;
		lvl.schody_dol_dir = opcje.schody_dol_kierunek;
		lvl.schody_dol_w_scianie = opcje.schody_dol_w_scianie;
...
void Game::GenerateDungeonObjects2()
{
	...
	{
		Object& o = Add1(local_ctx.objects);
		o.mesh = (lvl.schody_dol_w_scianie ? aSchodyDol2 : aSchodyDol);
		o.pos = pt_to_pos(lvl.schody_dol);
		o.rot = VEC3(0, dir_to_rot(lvl.schody_dol_dir), 0);
		o.scale = 1;
		o.base = NULL;
	}
...
void Game::DoWorldProgress(int days)
{
	...
	for(vector<Quest_Dungeon*>::iterator it = quests_timeout.begin(), end = quests_timeout.end(); it != end;)
	{
		if((*it)->IsTimedout() && (*it)->target_loc != current_location)
		{
			...
			if(loc->type == L_CAMP)
			{
				...
				for(vector<Location*>::iterator it2 = locations.begin(), end2 = locations.end(); it2 != end2; ++it)
				{
					if((*it2) == loc)
					{
						if(it2+1 == end2)
	...
	for(vector<Location*>::iterator it = locations.begin(), end = locations.end(); it != end; ++it, ++index)
	{
		...
		else if((*it)->last_visit != -1 && worldtime - (*it)->last_visit >= 30)
		{
			...
			{
				InsideLocation* inside = (InsideLocation*)*it;
				if(inside->cel != LABIRYNTH)
					inside->spawn = RandomSpawnGroup(g_base_locations[inside->cel]);
				if(inside->IsMultilevel())
...
int Game::GetClosestLocation(LOCATION type, const VEC2& pos, int cel)
{
	int best = -1, index = 0;
	float dist, best_dist;
	...
	for(vector<Location*>::iterator it = locations.begin(), end = locations.end(); it != end; ++it, ++index)
	{
		if(!*it || (*it)->active_quest || (*it)->type != type)
			continue;
		if(cel != -1 && ((InsideLocation*)(*it))->cel != cel)
			continue;
...
int Game::GetClosestLocationNotTarget(LOCATION type, const VEC2& pos, int nie_cel)
{
	int best = -1, index = 0;
	float dist, best_dist;
	...
	for(vector<Location*>::iterator it = locations.begin(), end = locations.end(); it != end; ++it, ++index)
	{
		if(!*it || (*it)->active_quest || (*it)->type != type)
			continue;
		if(((InsideLocation*)(*it))->cel == nie_cel)
			continue;
		dist = distance((*it)->pos, pos);
		if(best == -1 || dist < best_dist)
		{
	...
	for(vector<Location*>::iterator it = locations.begin(), end = locations.end(); it != end; ++it, ++index)
	{
		if(*it && IS_SET(flags, 1<<(*it)->type) && (!not_quest || !(*it)->active_quest))
		{
			if(flagi_cel != -1)
			{
				if((*it)->type == L_DUNGEON || (*it)->type == L_CRYPT)
				{
					if(!IS_SET(flagi_cel, 1<<((InsideLocation*)(*it))->cel))
						break;
				}
			}
