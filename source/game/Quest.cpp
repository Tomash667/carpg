...
void Quest_DostarczList::SetProgress(int prog2)
{
	...
	case 1:
		// koniec rozmowy z osob¹ daj¹c¹ questa
		{
			...
			game->journal->NeedUpdate(Journal::Quests, game->quests.size()-1);
	...
	case 1:
		{
			...
			if(!game->plotka_questowa[P_ORKOWIE])
			{
				...
				game->plotki.push_back(Format(game->journal->txAddNote, game->day+1, game->month+1, game->year, text));
				game->journal->NeedUpdate(Journal::Rumors);
