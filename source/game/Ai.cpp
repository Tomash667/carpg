...
void Game::UpdateAi(float dt)
{
	...
	for(vector<AIController*>::iterator it = ais.begin(), end = ais.end(); it != end; ++it)
	{
		...
		ai.pf_timer -= dt;
		{
			const float maxm = (IS_SET(u.data->flagi, F_TCHORZLIWY) ? 5.f : 10.f);
			ai.morale += dt;
			if(ai.morale > maxm)
				ai.morale = maxm;
		}
		...
		// uciekanie
		if(enemy && ai.state != AIController::Escape && !IS_SET(u.data->flagi, F_NIE_UCIEKA))
		{
			...
			for(int i=0; i<3; ++i)
			{
				if(u.data->spells && u.data->spells->spell[i] && IS_SET(u.data->spells->spell[i]->flags, Spell::NonCombat) && ai.cooldown[i] <= 0.f)
				{
					...
					if(IS_SET(u.data->spells->spell[i]->flags, Spell::Raise))
					{
						...
						for(vector<Unit*>::iterator it2 = ctx.units->begin(), end2 = ctx.units->end(); it2 != end2; ++it2)
						{
							if(!(*it2)->to_remove && (*it2)->live_state == Unit::DEAD && !IsEnemy(u, **it2) && IS_SET((*it2)->data->flagi, F_NIEUMARLY) &&
					...
					else
					{
						...
						for(vector<Unit*>::iterator it2 = ctx.units->begin(), end2 = ctx.units->end(); it2 != end2; ++it2)
						{
							if(!(*it2)->to_remove && !IsEnemy(u, **it2) && !IS_SET((*it2)->data->flagi, F_NIEUMARLY) && (*it2)->hpmax - (*it2)->hp > 100.f && 
								(dist = distance(u.pos, (*it2)->pos)) < spell_range && CanSee(u, **it2))
							{
			...
			case AIController::Idle:
				{
					if(u.useable == NULL)
					{
						if(ai.alert_target)
						{
							...
							else
							{
								...
								repeat = true;
								if(IS_SET(u.data->flagi2, F2_OKRZYK))
									AI_Shout(ctx, ai);
								break;
							}
						}
					...
					if(u.useable && u.useable->user != &u)
					{
						...
						WARN(Format("Invalid useable user: %s is using %s but the user is %s.", u.data->id, u.useable->GetBase()->id, u.useable->user ? u.useable->user->data->id : "NULL"));
						u.useable = NULL;
#ifdef IS_DEV
					...
					if(u.action == A_BLOCK)
					{
						...
						WARN(Format("Unit %s (%s) blocks in idle.", u.data->id, u.GetName()));
#ifdef IS_DEV
						...
						(u.action == A_ATTACK && ai.idle_action == AIController::Idle_TrainCombat)))
					{
						...
						if(ai.timer <= 0.f)
						{
							...
							else if(ai.idle_action == AIController::Idle_Look)
							{
								...
								if(IS_SET(u.data->flagi, F_AI_STRAZNIK) && angle_dif(u.rot, ai.start_rot) > PI/4)
									ai.idle_data.rot = ai.start_rot;
								else if(IS_SET(u.data->flagi2, F2_OGRANICZONY_OBROT))
							...
							else if(IS_SET(u.data->flagi3, F3_GORNIK) && rand2()%2 == 0)
							{
...
#define I_JEDZ 8
								...
								else if(IS_SET(u.data->flagi2, F2_UZYWA_TRONU) && !u.IsFollower())
									co = I_UZYJ;
								else if(u.type == Unit::HUMAN)
								{
									if(!IS_SET(u.data->flagi, F_AI_STRAZNIK))
									{
										if(IS_SET(u.data->flagi2, F2_AI_TRENUJE) && rand2()%5 == 0)
										{
											...
											{
												if(IS_SET(u.data->flagi3, F3_NIE_JE))
													co = rand2()%6;
												else
												{
													co = rand2()%7;
								...
								if(co == I_GADAJ && IS_SET(u.data->flagi3, F3_GADA_NA_ZAWODACH) && (chlanie_stan >= 3 || zawody_stan >= IS_ROZPOCZYNANIE))
									co = I_PATRZ;
								...
								case I_UZYJ:
									...
									if(ctx.useables)
									{
										...
										{
											Useable& use = **it2;
											if(!use.user && (use.type != U_TRON || IS_SET(u.data->flagi2, F2_UZYWA_TRONU)) && distance(use.pos, u.pos) < 10.f
								...
								case I_GADAJ:
									...
									{
										const float d = ((IS_SET(u.data->flagi, F_AI_STRAZNIK) || IS_SET(u.data->flagi, F_AI_STOI)) ? 1.5f : 10.f);
										close_enemies.clear();
										for(vector<Unit*>::iterator it2 = ctx.units->begin(), end2 = ctx.units->end(); it2 != end2; ++it2)
										{
											if((*it2)->to_remove || !(*it2)->IsStanding() || (*it2)->invisible || *it2 == &u)
								...
								case I_IDZ:
									...
									ai.idle_action = AIController::Idle_Move;
									ai.city_wander = false;
									if(IS_SET(u.data->flagi, F_AI_STOI))
									{
									...
									else if(distance2d(u.pos, use.pos) < PICKUP_RANGE)
									{
										if(angle_dif(clip(u.rot+PI/2), clip(-angle2d(u.pos, ai.idle_data.useable->pos))) < PI/4)
										{
											...
											if(!needed_item || u.HaveItem(needed_item) || u.slots[SLOT_WEAPON] == needed_item)
											{
												...
												if(use.type == U_KRZESLO && IS_SET(u.data->flagi, F_AI_URZEDNIK))
												{
													czyta_papiery = true;
			...
			case AIController::Fighting:
				{
							...
							if(u.data->spells->spell[i] && u.level >= u.data->spells->level[i] && ai.cooldown[i] <= 0.f)
							{
								...
								if(best_dist < s.range)
								{
									...
									if(IS_SET(s.flags, Spell::Drain))
									{
										...
										if(IS_SET(enemy->data->flagi2, F2_BRAK_KRWII))
											ok = false;
									}
					...
					if(u.IsHoldingBow() || IS_SET(u.data->flagi, F_MAG))
					{
					...
					{
						if(!IS_SET(u.data->flagi, F_MAG) && u.action != A_CAST && u.action != A_SHOOT)
						{
							look_at = LookAtTarget;
							move_type = MovePoint;
							target_pos = enemy->pos;
			...
			else if(move == 1)
			{
				...
				if(my_tile != target_tile)
				{
						...
						((ai.pf_state == AIController::PFS_WALKING || ai.pf_state == AIController::PFS_MANUAL_WALK) && target_tile != ai.pf_target_tile && ai.pf_timer <= 0.f))
					{
						...
						if(FindPath(ctx, my_tile, target_tile, ai.pf_path, !IS_SET(u.data->flagi, F_NIE_OTWIERA), ai.city_wander && city_ctx != NULL))
						{
							// path found
				...
				// otwieranie drzwi
				if(ctx.doors && !IS_SET(u.data->flagi, F_NIE_OTWIERA))
				{
...
void Game::AI_DoAttack(AIController& ai, Unit* target, bool w_biegu)
{
	...
	if(u.action == A_NONE && (u.ani->ani->head.n_groups == 1 || u.stan_broni == BRON_WYJETA) && ai.next_attack <= 0.f)
	{
		...
		if(!IS_SET(u.data->flagi, F_BRAK_POTEZNEGO_ATAKU))
		{
