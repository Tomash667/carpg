...
void CampGenerator::GenerateObjects()
{
	...
	for(int i = 0; i < 20; ++i)
	{
		...
		// ognisko
		if(game.SpawnObjectNearLocation(game.local_ctx, Rand() % 5 == 0 ? ognisko_zgaszone : ognisko, pt, Random(MAX_ANGLE)))
		{
			// namioty / pos³ania
			for(int j = 0, ile = Random(4, 7); j < ile; ++j)
			{
				...
				if(czy_namiot)
					game.SpawnObjectNearLocation(game.local_ctx, namiot, pt + Vec2(sin(kat), cos(kat))*Random(4.f, 5.5f), pt);
				else
					game.SpawnObjectNearLocation(game.local_ctx, poslanie, pt + Vec2(sin(kat), cos(kat))*Random(3.f, 4.f), pt);
			}
		}
	}
	...
	for(int i = 0; i < 100; ++i)
	{
		...
		if(ok)
		{
			...
			auto e = game.SpawnObjectNearLocation(game.local_ctx, obj, pt, Random(MAX_ANGLE), 2.f);
