...
struct NetChangePlayer
{
	enum TYPE
	{
		...
		TRAIN, // response to training [byte(id)-type (0-attribute, 1-skill, 2-tournament), byte(count)-stat type]
		...
		GAIN_STAT, // player gained attribute/skill [bool(id)-is skill; byte(count)-count; byte(a)-what]
		...
		ADD_PERK, // add perk to player [byte(id)-perk, int(count)-value]
