...
void Game::StartTutorial()
{
	...
	// map
	const int size = 24;
	lvl.map = new Pole[size * size];
	for(int y = 0; y < size; ++y)
	{
		for(int x = 0; x < size; ++x)
		{
			Pole& p = lvl.map[x + y * size];
			p.flags = 0;
			switch(mapa_t[x + y * size])
			{
			case ' ':
				p.type = PUSTE;
				break;
	...
	// obiekty
	dungeon_gen->SetFlags(lvl.map, size);
