...
bool Game::SaveGameSlot(int slot, cstring text)
{
	...
	else
	{
		...
		cfg.Add("player_class", Class::classes[(int)ss.player_class]->id.c_str());
		...
		for(int i = 1; i <= MAX_SAVE_SLOTS; ++i)
		{
			...
			if(io::FileExists(filename))
			{
				...
				if(io::FileExists(filename))
				{
					...
					if(str == "0")
						slot.player_class = Class::OldToNew(old::ClassId::Warrior);
					else if(str == "1")
						slot.player_class = Class::OldToNew(old::ClassId::Hunter);
					else if(str == "2")
						slot.player_class = Class::OldToNew(old::ClassId::Rogue);
					else
					{
						Class* c = Class::TryGet(str);
						if(c && c->IsPickable())
							slot.player_class = (ClassId)c->index;
						else
							slot.player_class = ClassId::None;
					}
				}
				else
				{
...
void Game::SaveGame(HANDLE file)
{
	...
	if(Net::IsOnline())
	{
		...
		WriteFile(file, &Effect::netid_counter, sizeof(Effect::netid_counter), &tmp, nullptr);
...
void Game::LoadGame(HANDLE file)
{
	...
	if(mp_load)
	{
		...
		if(LOAD_VERSION >= V_CURRENT)
			ReadFile(file, &Effect::netid_counter, sizeof(Effect::netid_counter), &tmp, nullptr);
